...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim_per_material, hidden_size):
        super(ActorCritic, self).__init__()
        self.state_dim = state_dim
        self.action_dim_per_material = action_dim_per_material
        ...
        # Actor heads (one per material)
        self.actor_heads = nn.ModuleList([
            nn.Linear(hidden_size, action_dim_per_material) for _ in range(MAT_COUNT)
        ])
...
class PPO:
    def __init__(self, state_dim, action_dim_per_material, hidden_size, lr):
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() else "cpu")
        self.model = ActorCritic(
            state_dim, action_dim_per_material, hidden_size).to(self.device)
    ...
    def compute_gae(self, rewards, values, next_value, masks, gamma, lam):
    ...
    def update(self, states, actions, log_probs_old, returns, advantages):
        ...
        for _ in range(PPO_EPOCHS):
            ...
            for i, logits in enumerate(action_logits):
                ...
                ratio = torch.exp(log_probs - log_probs_old[:, i])
                surr1 = ratio * advantages
                surr2 = torch.clamp(ratio, 1 - PPO_CLIP,
                                    1 + PPO_CLIP) * advantages
...
#     def __init__(self, state_dim: int, action_dims: List[int], hidden_layer_sizes: List[int] = [64, 64]):
#         """
...
#         Args:
#             state_dim (int): Dimension of the state space
#             action_dims (List[int]): Dimensions of the action space
#             hidden_layer_sizes (List[int]): Sizes of hidden layers
...
#             critic_discount (float): Value function coefficient
#             entropy_beta (float): Entropy coefficient
#             epochs (int): Number of epochs to train on the collected data
...
#         # Make sure action is returned as an array, even for a single action
#         action_numpy = action.cpu().numpy()
#         # If action is just a 1D array (for a single sample)
#         if action_numpy.ndim == 1:
#             action_numpy = action_numpy.reshape(-1)  # Ensure it's a 1D array
#         else:
#             action_numpy = action_numpy[0]  # Get the first sample from batch
