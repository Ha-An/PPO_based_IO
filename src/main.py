...
def main():
    ...
    action_dim_per_material = ACTION_MAX + 1  # 0 to 5
    agent = PPO(state_dim, action_dim_per_material, HIDDEN_SIZE, LEARNING_RATE)

    # Train the agent
    wrapper.train(agent, NUM_EPISODES)

    # Evaluate the agent
    wrapper.evaluate(agent)
...
#     print(f"Action Space: MultiDiscrete({[ACTION_MAX + 1] * MAT_COUNT})")
#     print(f"State Dimensions: {len(STATE_MINS)}")
